...
- 複数局を登録できるインターネットラジオ
- 休憩画面での手動選局、または開始時ランダム選局・作業中固定モード
...
codex-rest reset      # アーカイブ等で残ったタスク状態を解除
codex-rest doctor     # Chrome、フック、通知音などを診断
codex-rest uninstall  # 本体と追加フックを削除（設定・音源は保持）
```

作業中のスレッドがアーカイブされるなどして `Stop` フックが届かなかった場合、
設定画面の「状態をリセット」または `codex-rest reset` で残ったタスクを解除できます。
同じCodexセッションで次のターンが始まった場合は、古いターンを自動的に回収します。
...
ラジオ局は音声ストリームの直接HTTP/HTTPS URLを設定画面から登録します。
音声はChromeから局へ直接接続するため、局のサーバーには通常のインターネットラジオ再生と同様にIPアドレス等が送信されます。
局側のストリーム形式や公開状態によっては再生できない場合があり、その場合は別の局を試した後に内蔵曲へフォールバックします。
